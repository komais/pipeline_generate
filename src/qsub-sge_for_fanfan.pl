...
=head1 Usage
  ...
  --continue        continue running unfinished job and skip finished job , default yes      
  ...
  --du              check analysis dir size , default no du 
...
my ($quota , $Analysis_dir , $max_cycle , $du); 
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
readPreLogs($work_shell_file , \%finish_blocks ) if ( $Continue eq 'yes'); ## read log file and store finish job 
